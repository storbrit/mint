package org.storbrit.mint.augredstone.block;
...
public class AugmentedRedstoneWireBlock extends Block {
	...
	public AugmentedRedstoneWireBlock(AbstractBlock.Settings settings) {
		super(settings);
	...
	private VoxelShape getShapeForState(BlockState state) {
		...
		for (Direction direction : Direction.Type.HORIZONTAL) {
			...
			if (wireConnection == WireConnection.SIDE) {
				voxelShape = VoxelShapes.union(voxelShape, SHAPES_FLOOR.get(direction));
			} else if (wireConnection == WireConnection.UP) {
				voxelShape = VoxelShapes.union(voxelShape, SHAPES_UP.get(direction));
			}
		}
	...
	private BlockState getPlacementState(BlockView world, BlockState state, BlockPos pos) {
		...
		state = this.getMissingConnections(world, this.getDefaultState().with(POWER, state.get(POWER)), pos);
	...
	private BlockState getMissingConnections(BlockView world, BlockState state, BlockPos pos) {
		...
		for (Direction direction : Direction.Type.HORIZONTAL) {
			if (!state.get(DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(direction)).isConnected()) {
				...
				state = state.with(DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(direction),
						wireConnection);
			}
		}
	...
	public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState neighborState, WorldAccess world, BlockPos pos, BlockPos neighborPos) {
		if (direction == Direction.DOWN) {
			...
			return wireConnection.isConnected() == (state.get(DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(direction))).isConnected() && !isFullyConnected(state) ?
					state.with(
							DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(direction), wireConnection) : this.getPlacementState(world,
					this.dotState.with(POWER, state.get(POWER))
							.with(DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(direction), wireConnection), pos);
		}
	}

	private static boolean isFullyConnected(BlockState state) {
		return state.get(WIRE_CONNECTION_NORTH).isConnected() && state.get(WIRE_CONNECTION_SOUTH).isConnected() && state.get(WIRE_CONNECTION_EAST)
				.isConnected() && state.get(WIRE_CONNECTION_WEST).isConnected();
	}
		...
		for (Direction direction : Direction.Type.HORIZONTAL) {
			...
			if (wireConnection != WireConnection.NONE && !world.getBlockState(mutable.set(pos, direction)).isOf(this)) {
				...
				if (blockState.isOf(this)) {
					...
					world.updateNeighbor(direction.getOpposite(), world.getBlockState(blockPos), mutable, blockPos,
				...
				if (blockState2.isOf(this)) {
					...
					world.updateNeighbor(direction.getOpposite(), world.getBlockState(blockPos2), mutable, blockPos2,
							flags, maxUpdateDepth);
				}
			}
		}
	...
	private WireConnection getRenderConnectionType(BlockView world, BlockPos pos, Direction direction, boolean isNotSolidBlock) {
		BlockPos blockPos = pos.offset(direction);
		BlockState blockState = world.getBlockState(blockPos);
		if (isNotSolidBlock) {
			boolean bl = this.canRunOnTop(world, blockPos, blockState);
	...
	private void addPoweredParticles(World world, RandomGenerator random, BlockPos pos, Vec3d color, Direction direction, Direction direction2, float f, float g) {
		float h = g - f;
		if (!(random.nextFloat() >= 0.2F * h)) {
			...
			world.addParticle(new DustParticleEffect(color.m_sruzucpd(), 1.0F), (double)pos.getX() + d, (double)pos.getY() + e, (double)pos.getZ() + k, 0.0, 0.0, 0.0);
		}
	}
	...
	static {
		...
		DIRECTION_TO_WIRE_CONNECTION_PROPERTY = Maps.newEnumMap(
				ImmutableMap.of(Direction.NORTH, WIRE_CONNECTION_NORTH, Direction.EAST, WIRE_CONNECTION_EAST, Direction.SOUTH, WIRE_CONNECTION_SOUTH, Direction.WEST, WIRE_CONNECTION_WEST));
		...
		SHAPES_UP = Maps.newEnumMap(ImmutableMap.of(Direction.NORTH, VoxelShapes.union(
				SHAPES_FLOOR.get(Direction.NORTH), Block.createCuboidShape(3.0, 0.0, 0.0, 13.0, 16.0, 1.0)), Direction.SOUTH, VoxelShapes.union(
				SHAPES_FLOOR.get(Direction.SOUTH), Block.createCuboidShape(3.0, 0.0, 15.0, 13.0, 16.0, 16.0)), Direction.EAST, VoxelShapes.union(
				SHAPES_FLOOR.get(Direction.EAST), Block.createCuboidShape(15.0, 0.0, 3.0, 16.0, 16.0, 13.0)), Direction.WEST, VoxelShapes.union(
				SHAPES_FLOOR.get(Direction.WEST), Block.createCuboidShape(0.0, 0.0, 3.0, 1.0, 16.0, 13.0))));
		SHAPES = Maps.newHashMap();
		COLORS = Util.make(new Vec3d[16], (vec3ds) -> {
			for(int i = 0; i <= 15; ++i) {
				float f = (float)i / 15.0F;
				float g = MathHelper.clamp(f * f * 0.7F - 0.5F, 0.0F, 1.0F);
				float h = MathHelper.clamp(f * f * 0.6F - 0.7F, 0.0F, 1.0F);
				float j = f * 0.6F + (f > 0.0F ? 0.4F : 0.3F);
